...
from agent.nodes import PlannerOutput, call_model, planner, responder, tool_node
...
class TestPlannerNode:
    ...
    def test_planner_success(self, mock_get_llm, base_state):
        ...
        mock_structured_llm.invoke.return_value = PlannerOutput(
            search_terms=["term1"], date_from="2021-01-01", date_to="2024-01-01", top_k=10
        )
...
class TestAgentNodes:
    ...
    def test_call_model_tool_choice(self, mock_get_llm, base_state):
        ...
        mock_llm.invoke.return_value = AIMessage(
            content="", tool_calls=[{"name": "search_arxiv", "args": {"query": "test"}, "id": "1"}]
        )
    ...
    def test_tool_node_execution(self, mock_search, base_state):
        base_state["messages"] = [
            AIMessage(
                content="",
                tool_calls=[{"name": "search_arxiv", "args": {"query": "test"}, "id": "1"}],
            )
        ]

...
def get_citation_count(paper: Paper) -> Paper:
    ...
    if paper.citation_count is None:
        log.warning("semantic_scholar.not_found", title=paper.title, arxiv_id=paper.arxiv_id, doi=paper.doi)
        
    return paper

def _get_by_id(client: httpx.Client, paper_id: str) -> Optional[dict]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log.debug("semantic_scholar.api_status", status_code=response.status_code, paper_id=paper_id)
            return None
    except Exception as e:
        log.debug("semantic_scholar.id_lookup_failed", paper_id=paper_id, error=str(e))
        return None

def _search_by_title(client: httpx.Client, title: str) -> Optional[dict]:
...
def _update_paper(paper: Paper, result: dict, method: str) -> Paper:
    ...
    log.info("semantic_scholar.match_found", 
             title=paper.title, 
             method=method, 
